...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        new_s = [start] * window_size + sentence + [end] * window_size
        head, tail = window_size, window_size + len(sentence) - 1
        for i in range(head, tail + 1):
            window = new_s[i-window_size: i+window_size+1]
            window = sum(window, [])
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding = tf.nn.embedding_lookup(word_embedding, self.input_placeholder)
        embeddings = tf.reshape(embedding, [-1, Config.n_window_features * Config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        w1 = tf.get_variable(name='w1', shape=[self.config.n_windows_features*self.config.embed_size, self.config.hidden_size]
                             , dtype=tf.float32, initializer=tf.contrib.layer.xavier_initializer())
        b1 = tf.Variable(tf.zeros(shape=Config.hidden_size,))
        w2 = tf.get_variable(name='w2', shape=[self.config.hidden_size, self.config.n_classes]
                             , dtype=tf.float32, initializer=tf.contrib.layer.xavier_initializer())
        b2 = tf.Variable(tf.zeros(shape=Config.n_classes,))
