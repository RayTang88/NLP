...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        w1 = tf.Variable(xinit((self.config.n_features * self.config.embed_size, self.config.hidden_size)))
        w2 = tf.Variable(xinit((self.config.hidden_size, self.config.n_classes)))
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))
        b2 = tf.Variable(tf.zeros([self.config.n_classes]))
        h1 = tf.nn.relu(tf.add(tf.matmul(x, w1), b1))
        h2 = tf.nn.dropout(h1, self.dropout_placeholder)
        pred = tf.add(tf.matmul(h2, w2), b2)
    ...
    def add_loss_op(self, pred):
        ...
        prob = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
